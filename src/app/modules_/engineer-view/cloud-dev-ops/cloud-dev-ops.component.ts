...
export class CloudDevOpsComponent implements OnInit {
  ...
  getAllEngineer(){
    this.clouddevopsservice.getAllEngineer().subscribe((res)=>{
      if(res['status']){
        this.allEngineer=res['data'];
      }
    })
  }
    getAllSituation(){
    ...
    selectedEngineer;
    selectEngineer(checked,engineer){
      console.log(checked);
    this.selectedEngineer=engineer;
    }
    assignEpisode(){
   this.clouddevopsservice.assignEpisode(this.selectedEngineer.name,this.selectedEpisode.id).subscribe((res)=>{
    this.getAllSituation();
    swal(res['msg']);
   });
    }
    assignOwn(id){
      this.clouddevopsservice.assignEpisode('vikas',id).subscribe((res)=>{
        this.getAllSituation();
        swal(res['msg']);
       });
    }
  open(content ) {
   ...
   updateEpisode(episodeid,status){
     this.clouddevopsservice.updateEpisode(episodeid,status).subscribe((res)=>{
      this.getAllSituation(); 
      swal(res['msg']);
     })
   }
